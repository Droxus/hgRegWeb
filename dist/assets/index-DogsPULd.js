...
и возврат средств и т.д. и т.п.`};function hl(n){const t=document.getElementById(n);for(;t.firstChild;)t.removeChild(t.firstChild)}function kp(){Array.from(document.getElementsByClassName("serviceBtns")).forEach(n=>{n.addEventListener("click",()=>{const t=n.getAttribute("service-name");Kt=Yn[t],Gp()})})}function Op(){kp()}function Vt(n,t,e){Array.from(document.getElementsByClassName(n)).forEach(r=>{r.addEventListener(t,e)})}function Mp(n){this.style.height="auto",this.style.height=this.scrollHeight+"px";const t=n.target,e=t.value.length,r=t.getAttribute("maxlength"),s=t.parentElement.getElementsByClassName("inputHelper")[0];e>r&&(t.value=t.value.substring(0,r)),s.textContent=`${t.value.length}/${r} characters used`}function Lp(n){if(n){const t=n.parentElement.getElementsByClassName("inputError")[0];t&&(console.log("show error"),t.style.display="block"),n.style.outline="1px solid "+cl,n.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{t.style.display="none",n.style.outline="none"},3e3)}}function Fp(){const n=ul[Kt];if(n==null)return;const t={};for(const[e,r]of Object.entries(n)){const s=bp(r),o=document.getElementById(r.id);if(s)t[e]=o.value??"";else{Lp(o);return}}return t}let Ao;function Bp(){clearTimeout(Ao),Ao=setTimeout(()=>{document.getElementById("submitErrorLbl").style.color="transparent"},5e3),document.getElementById("submitErrorLbl").style.color=cl}function Up(){hl("app");const e=Kt==Yn.BASIC||Kt==Yn.ADVANCED?`Спасибо! <br>Мы свяжемся с вами сразу как только получим для вас точную дату подачи 
  ...
  `),document.getElementById("updPagebtn").addEventListener("click",()=>{location.reload()})}async function $p(){console.log("submit btn clicked");const n=Fp();n==null&&Bp(),console.log(n),await wp(n,Kt),Up()}function jp(){Vt("passport","input",Cp),Vt("onlyDigits","input",Rp),Vt("onlyText","input",Pp),Vt("onlyAllText","input",Sp),Vt("phone","input",Np),Vt("email","input",Vp),Vt("date","keydown",Io),Vt("date","input",Io),Vt("textarea","input",Mp),document.getElementById("submitBtn").addEventListener("click",$p)}function qp(n){const{id:t,fieldName:e,type:r,className:s,labelName:o,placeholder:l,maxlength:c,minlength:d,required:f,inputHelper:I}=n;return r=="textarea"?`
  ...
  `;n.insertAdjacentHTML("beforeend",t)}function zp(){hl("app"),document.getElementById("app").insertAdjacentHTML("beforeend",'<div id="formContainer"></div>'),Hp();const n=ul[Kt]??{},t=document.getElementById("formContainer");Object.entries(n).forEach(([e,r])=>{const s=qp(r);t.insertAdjacentHTML("beforeend",s)}),t.insertAdjacentHTML("beforeend",`
